...
from app.schemas.operations import RouteCreateRequest, RoutePlanRequest, RouteUpdateRequest, VisitCreateRequest, VisitNoteRequest, VisitUpdateRequest
...
class RoutePlanningService:
    ...
    async def plan(self, organization_id: UUID, payload: RoutePlanRequest) -> dict:
        query = select(HCP).where(HCP.organization_id == organization_id, HCP.status == "active", HCP.deleted_at.is_(None))
        if payload.hcp_ids:
            query = query.where(HCP.id.in_(payload.hcp_ids))
        doctors = list((await self.session.scalars(query)).all())
        priority_engine = PriorityCalculationEngine()
        candidates = []
        for doctor in doctors:
            priority = await priority_engine.calculate(self.session, organization_id, doctor.id)
            address = await self.session.scalar(select(Address).where(Address.organization_id == organization_id, Address.hcp_id == doctor.id, Address.latitude.is_not(None), Address.longitude.is_not(None), Address.deleted_at.is_(None)).order_by(Address.created_at))
            follow_up = await self.session.scalar(select(Visit.follow_up_at).where(Visit.organization_id == organization_id, Visit.hcp_id == doctor.id, Visit.follow_up_at.is_not(None), Visit.deleted_at.is_(None)).order_by(Visit.follow_up_at.desc()))
            scheduled = await self.session.scalar(select(Visit.scheduled_at).where(Visit.organization_id == organization_id, Visit.hcp_id == doctor.id, Visit.status == "scheduled", Visit.deleted_at.is_(None)).order_by(Visit.scheduled_at))
            urgency = (20 if follow_up and follow_up <= payload.workday_end else 0) + (30 if scheduled and scheduled.date() == payload.route_date else 0)
            candidates.append({"doctor": doctor, "priority": priority, "urgency": urgency, "address": address})
        current = (payload.origin_latitude, payload.origin_longitude) if payload.origin_latitude is not None and payload.origin_longitude is not None else None
        ordered, remaining = [], candidates[:]
        while remaining:
            def rank(item):
                point = (item["address"].latitude, item["address"].longitude) if item["address"] else None
                distance = self.distance.kilometers(current, point) if current and point else 9999
                return -(item["priority"].total_score + item["urgency"] * 2) + distance * 0.15
            chosen = min(remaining, key=rank)
            ordered.append(chosen); remaining.remove(chosen)
            if chosen["address"]: current = (chosen["address"].latitude, chosen["address"].longitude)
        cursor, total_distance = payload.workday_start, 0.0
        stops = []
        previous = (payload.origin_latitude, payload.origin_longitude) if payload.origin_latitude is not None and payload.origin_longitude is not None else None
        for index, item in enumerate(ordered, 1):
            point = (item["address"].latitude, item["address"].longitude) if item["address"] else None
            travel = max(0, round(self.distance.kilometers(previous, point) / 40 * 60)) if previous and point else 0
            total_distance += self.distance.kilometers(previous, point) if previous and point else 0
            cursor += timedelta(minutes=travel)
            arrival = cursor
            cursor += timedelta(minutes=30)
            stops.append({"sequence": index, "hcp_id": str(item["doctor"].id), "name": f"{item['doctor'].first_name} {item['doctor'].last_name}", "priority_score": item["priority"].total_score, "priority_factors": item["priority"].factors, "urgency_score": item["urgency"], "travel_minutes": travel, "planned_arrival_at": arrival, "planned_departure_at": cursor, "latitude": point[0] if point else None, "longitude": point[1] if point else None})
            previous = point or previous
        return {"route_date": payload.route_date, "distance_km": round(total_distance, 2), "total_minutes": round((cursor - payload.workday_start).total_seconds() / 60), "stops": stops, "map_stops": [{key: stop[key] for key in ("hcp_id", "sequence", "latitude", "longitude")} for stop in stops]}
...
class CalendarService:
    ...
    async def events(self, organization_id: UUID, user_id: UUID, starts_at: datetime, ends_at: datetime) -> list[dict]:
        ...
        hcp_ids = {v.hcp_id for v in visits}
        doctors = {d.id: d for d in (await self.session.scalars(select(HCP).where(HCP.id.in_(hcp_ids)))).all()} if hcp_ids else {}
        return ([{"id": str(v.id), "type": "visit", "title": f"Visit: {doctors[v.hcp_id].first_name} {doctors[v.hcp_id].last_name}" if v.hcp_id in doctors else "Scheduled doctor visit", "starts_at": v.scheduled_at, "ends_at": v.follow_up_at or v.scheduled_at, "hcp_id": str(v.hcp_id), "hcp_name": f"{doctors[v.hcp_id].first_name} {doctors[v.hcp_id].last_name}" if v.hcp_id in doctors else None, "status": v.status} for v in visits] + [{"id": str(e.id), "type": e.event_type, "title": e.title, "starts_at": e.starts_at, "ends_at": e.ends_at, "hcp_id": str(e.hcp_id) if e.hcp_id else None} for e in events])
