...
async def postcall(payload: PostCallRequest, session: AsyncSession = Depends(get_session), user: UserMe = Depends(require_permissions("ai:use"))): return await AIOrchestratorService(session).postcall(payload)
@router.post("/summary")
