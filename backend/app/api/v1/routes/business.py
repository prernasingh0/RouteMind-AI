...
def register_crud(path: str, model: Any, create_schema: Any, update_schema: Any, read_schema: Any, permission_prefix: str, service_cls: Any = TenantCrudService, user_scoped: bool = False, include_create: bool = True, include_update: bool = True, include_delete: bool = True) -> None:
    ...
    async def list_items(limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), sort: str = "created_at", search: str | None = None, session: AsyncSession = Depends(get_session), user: UserMe = Depends(require_permissions(f"{permission_prefix}:read"))):
        item_service = service(session)
        if user_scoped:
            return await item_service.list_for_user(user.organization_id, user.id, limit=limit, offset=offset, sort=sort, search=search)
        return await item_service.list(user.organization_id, limit=limit, offset=offset, sort=sort, search=search)
...
register_crud("/doctors", models.HCP, s.HCPCreate, s.HCPUpdate, s.HCPRead, "doctors", DoctorService, include_create=False, include_update=False, include_delete=False)
...
register_crud("/visits", models.Visit, s.VisitCreate, s.VisitUpdate, s.VisitRead, "visits", VisitService, include_create=False, include_update=False, include_delete=False)
...
register_crud("/routes", models.Route, s.RouteCreate, s.RouteUpdate, s.RouteRead, "routes", RouteService, include_create=False, include_update=False)
...
register_crud("/notifications", models.Notification, s.NotificationCreate, s.NotificationUpdate, s.NotificationRead, "notifications", NotificationService, user_scoped=True)
